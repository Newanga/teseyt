      ...
      <div className="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8 ">
        ...
        <div className="px-4 py-4 sm:px-0 grid sm:grid-cols-2 gap-4 md:grid-cols-3">
          {CATEGORIES.map((category) => (
            <div
              key={category.id}
              className="relative h-20 mb-8 border rounded-lg shadow-red"
            >
              <img
                className="opacity-50 object-cover h-20 w-full rounded-lg"
                src={category.imgUrl}
                alt={category.imgAlt}
              />
              <h1 class="absolute text-3xl top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                {" "}
                {category.name}
              </h1>
            </div>
